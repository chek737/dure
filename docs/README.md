...
- [모델 선택 정책](model-selection.md): GPU 인벤토리를 바탕으로 한 결정론적 선택기, 모델 레지스트리와 중앙 추천 설계 — 부분 구현
- [벤치마크 및 모델 자격 검증](benchmarking.md): 후보 모델의 성능·안정성 승격 기준 — 계획됨
...
- [개발 로드맵](roadmap.md): v0.3.5 이후의 단계별 목표와 완료 기준
...
- 모델 카탈로그의 실제 상태는 향후 레지스트리가 진실의 원천이 되며, 이 문서는 정책과 절차를 설명합니다.
