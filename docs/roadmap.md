...
기준일: 2026-07-20
현재 개발 브랜치: `version/0.3.10` (안전한 단일 노드 Agent 벤치마크 작업)
...
- `version/0.3.11` 이후: 추천 승인·배포 세대, 명시적 롤백

현재 `version/0.3.10` 범위는 관리자가 등록한 증적의 판정·승격 게이트에 더해, 승인된 단일 노드에서 네 가지 고정 작업 부하를 실행하고 구조화된 증적으로 수집합니다. 준비 단계는 작업이나 호스트 변경을 만들지 않고, 명시적 적용 뒤에도 로컬 캐시와 다이제스트 이미지로 격리된 벤치마크만 실행합니다. 전체 작업 부하 매트릭스, 다중 노드 네트워크·NCCL과 24시간·장애 복구 수용 검사는 구현하지 않습니다.
