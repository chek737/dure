...
기준일: 2026-07-21
현재 누적 개발 브랜치: `version/0.3.14` (정확한 다중 노드 증적 자격과 불변 아티팩트 매니페스트, 공식 병합 전 Draft)
...
- 후속 버전: 콘텐츠 주소 기반 캐시 준비, 명시적 중앙 준비, 단계별 아티팩트와 결정론적 다중 노드 런타임

현재 누적 `version/0.3.14` 범위는 추천과 수락의 무변경 경계를 유지하면서 apply·verify operation, 노드별 상태와 task 시도 번호를 기록합니다. 계획의 전체 배정 노드가 모두 검증에 성공하고 모든 Agent가 0.3.12 이상일 때만 `verified_at`을 기록합니다. 최신 세대는 직접 연결된 상태가 `VERIFIED`이고 `verified_at`을 보유한 직전 세대로만 롤백할 수 있으며, 기본 요청은 준비만 하고 `apply=true`에서만 호스트 변경을 시작합니다. 다중 노드 추천은 정확한 노드 조합에 결합된 최신 네트워크·NCCL 증적을 요구하며, 아티팩트 매니페스트 등록은 DB에 정규 파일·청크 관계만 저장하고 아직 다운로드나 노드 캐시 준비를 수행하지 않습니다. 이 두 버전 브랜치는 공식 `main`에 병합되기 전까지 Draft 개발 상태입니다.
