...
기준일: 2026-07-20
현재 개발 브랜치: `version/0.3.9` (벤치마크 증적과 모델 승격 게이트 작업)
...
- `version/0.3.10` 이후: 폐쇄형 Agent 벤치마크, 추천 승인·배포 세대, 명시적 롤백

현재 `version/0.3.9` 범위는 관리자가 등록한 증적의 고정 식별자와 SLO를 검사하고, 모든 배치 프로필의 최신 결과가 통과한 릴리스만 `ACTIVE`로 승격합니다. GPU에서 벤치마크를 실행하는 Agent 작업, 전체 작업 부하 매트릭스, 다중 노드 24시간·장애 복구 수용 검사는 구현하지 않습니다.
...
현재 상태: 결정론적 선택기, 중앙 모델 레지스트리, 저장 인벤토리 기반 읽기 전용 추천, 구조화된 벤치마크 증적과 `ACTIVE` 승격 게이트까지 구현되었습니다. 추천 승인·배포 세대와 저장된 네트워크/NCCL 증적을 추천 자격에 연결하는 작업이 남아 있어 다중 노드 추천 완료 기준은 아직 충족하지 않았습니다.
...
- 벤치마크 실행은 폐쇄형 Agent 작업과 허용 목록 기반 실행기로 별도 구현하고, 증적 등록만으로 호스트 변경이 일어나지 않게 한다.
