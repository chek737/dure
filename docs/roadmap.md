...
기준일: 2026-07-20
현재 개발 브랜치: `version/0.3.12` (세대별 적용·검증 상태와 명시적 직전 세대 롤백)
...
- 후속 버전: 다중 노드 네트워크·NCCL 자동 증적, 24시간 복구 검증과 실제 환경 운영 강화

현재 `version/0.3.12` 범위는 추천과 수락의 무변경 경계를 유지하면서 apply·verify operation, 노드별 상태와 task 시도 번호를 기록합니다. 계획의 전체 배정 노드가 모두 검증에 성공하고 모든 Agent가 0.3.12 이상일 때만 `verified_at`을 기록합니다. 최신 세대는 직접 연결된 `verified_at` 보유 직전 세대로만 롤백할 수 있으며, 기본 요청은 준비만 하고 `apply=true`에서만 호스트 변경을 시작합니다.
