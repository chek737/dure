...
> 상태: **생성·등록·검증 계층 구현, 노드 배포 소비는 계획됨**. Dure 0.3.17은 제한된 vLLM 계약으로 rank별 `STAGE` 아티팩트를 만드는 신뢰된 오프라인 빌더와, 그 결과를 `DRAFT`·`VALIDATED`·`REVOKED`로 관리하는 중앙 레지스트리를 제공합니다. Agent가 이 아티팩트를 내려받아 활성화하거나 추천·준비·배포가 `STAGE`를 선택하는 기능은 아직 제공하지 않습니다.
...
이 계층은 아티팩트를 생성하고 그 동일성·검증 상태를 기록합니다. 다음 작업은 하지 않습니다.

- GPU 노드에 stage 파일 다운로드 또는 P2P 전송
- Agent의 `STAGE` 캐시 materialize·활성화
- 추천이나 배포 준비에서 stage variant 자동 선택
- 실행 계획을 stage manifest와 결합하거나 vLLM 컨테이너 시작
- 기존 배포의 중지·교체·롤백

따라서 `VALIDATED` variant가 있다는 사실만으로 모델이 어떤 GPU 노드에 설치됐거나 실행 중이라고 판단해서는 안 됩니다.
...
builder와 중앙 레지스트리는 PP 1~64의 구조를 표현하지만, PR5에 포함된 실제 GPU acceptance harness는 `PP=1`만 native export·load·추론까지 검증합니다. PP>1 variant는 구조 검사와 `DRAFT` 등록까지 가능하더라도, 모든 rank 경로를 실제로 load하고 attest하는 별도의 신뢰된 검증이 없으면 `VALIDATED`로 운영하지 않습니다. Dure가 제공하는 PP>1 load/rank attestation은 후속 PR6~7 범위입니다.
...
다음 단계에서는 현재 레지스트리의 `VALIDATED` variant를 실제 배포 계층이 소비하도록 별도의 안전 검사를 추가해야 합니다.

1. 추천·수락이 정확한 source/runtime/topology에 맞는 `VALIDATED` variant만 선택합니다.
2. 중앙 준비 계획이 node UUID와 PP rank를 stage 매니페스트에 고정합니다.
3. Agent가 기존 콘텐츠 주소 저장소로 자신의 stage만 내려받아 `STAGE` cache를 원자적으로 활성화합니다.
4. 준비 증적이 배포 세대·노드·rank·stage manifest·exact path·OCI digest에 결합됩니다.
5. 모든 rank의 준비가 성공한 뒤에만 런타임 계획이 `sharded_state`를 사용합니다.
6. 실패 시 새 세대를 시작하지 않고 기존 검증 배포를 유지하며, 명시적 재시도·철회·롤백 절차를 제공합니다.
