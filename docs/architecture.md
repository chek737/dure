...
- 중앙 제어면은 노드 프로필, 관측·희망 상태, 배포 구성, 작업, 모델 레지스트리·벤치마크 증적, 자격 증명과 감사 이벤트를 저장합니다.
...
중앙 모델 레지스트리는 고정된 모델 아티팩트, OCI 런타임 이미지, 모델 릴리스와 배치 프로필을 저장합니다. `VALIDATED` 릴리스의 구조화된 벤치마크 증적은 다음 흐름으로 처리합니다.
...
최신 실패 증적은 과거 통과 결과보다 우선합니다. 승격 시 사용한 증적 집합을 릴리스에 고정해 반복 요청이 같은 결과를 내게 합니다. 이 흐름은 중앙 DB만 변경하며 deployment, task, 다운로드 또는 컨테이너 작업을 만들지 않습니다.

현재 증적은 신뢰된 관리 도구가 제출한 요약입니다. Agent가 GPU 벤치마크·네트워크 시험·NCCL을 실행하는 작업 유형은 아직 없으며, 전체 작업 부하 매트릭스와 다중 노드 24시간 복구 검증도 중앙 제어면이 자동 수행하지 않습니다.
...
추천기는 모델의 이름·크기만 비교하지 않고 GPU VRAM, 드라이버·연산 능력·런타임 지원 아키텍처, 디스크, 모델 아티팩트, 런타임 이미지, 컨텍스트·동시성, 네트워크/NCCL 조건을 평가합니다. 구조화된 네트워크·NCCL 증적 저장은 구현되었지만 추천기가 이를 노드 조합의 적격성 입력으로 조회하는 연계는 아직 없어 모든 중앙 다중 노드 후보를 실패 안전 방식으로 거부합니다. 모델 레지스트리, 아티팩트 검증, 세대별 단계적 전환의 상세 정책은 [모델 선택 정책](model-selection.md)과 [벤치마크 문서](benchmarking.md)를 따릅니다.
...
벤치마크 증적 등록과 모델 승격은 이 작업 프로토콜에 새 실행 명령을 추가하지 않습니다. 폐쇄형 `BENCHMARK` 작업과 허용 목록 기반 실행기는 후속 구현 범위입니다.
