...
| 모델 아티팩트 변경 | 리비전 고정 정책, 불변 정규 매니페스트와 파일·청크 SHA-256 레지스트리 | 실제 다운로드 바이트 검증, 게시자 서명과 provenance 검증 |
...
- 콘텐츠 주소 기반 다운로드, 실제 청크 해시 검증, 안전한 파일 조립과 캐시 승격은 후속 버전의 별도 실행 경계입니다.
