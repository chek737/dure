...
| 벤치마크 증적 바꿔치기 | 릴리스·배치·노드 UUID·현재 프로필 지문과 고정 아티팩트·런타임 식별자 결합 | 서명된 Agent 결과와 원본 증적 provenance |
...
## 모델 레지스트리, 승격 게이트와 계획된 추천기의 경계

모델 레지스트리의 영속 스키마, 관리자 인증 API, 고정 리비전·다이제스트 검증, 구조화된 벤치마크 증적과 `ACTIVE` 승격 게이트는 구현되었습니다. 정책 기반 추천은 아직 구현 중이며 호스트 변경 권한이 아닙니다. 향후 추천은 승인된 최신 인벤토리와 `ACTIVE` 모델 릴리스만 평가하고, 배포 구성 생성과 적용은 별도 운영자 행동으로 유지합니다.
...
현재 벤치마크 수치는 관리자 인증을 가진 신뢰된 외부 도구가 제출합니다. 중앙 제어면은 GPU나 네트워크 시험을 직접 실행하지 않고, 제출자가 전체 작업 부하 매트릭스 또는 24시간 복구 검증을 수행했는지도 증명하지 않습니다. 폐쇄형 Agent 작업, 결과 서명과 provenance 검증이 추가되기 전에는 이 API를 신뢰된 운영자 경계 밖에 노출하지 않습니다.
