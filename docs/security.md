...
- 적용 직전에 승인·온라인·신선한 인벤토리·디스크, 등록 매니페스트와 OCI 이미지 다이제스트를 다시 검사합니다. 불확실한 항목은 허용으로 추론하지 않습니다.
- 모델 단계의 전체 해시·marker·exact path 증적이 성공한 노드에만 이미지 단계를 만듭니다.
...
이 준비는 각 노드에 동일한 모델 전체 `FULL_SNAPSHOT`을 둡니다. rank별 `STAGE` 파일 생성, 노드 간 전송, 분산 빌더와 실제 NCCL 실행은 지원하지 않으므로 다중 노드 파일 분할의 보안·무결성 경계를 제공한다고 해석해서는 안 됩니다.
