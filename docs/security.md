...
- 중앙 추천과 선택기의 기본 정책은 네트워크·NCCL 증적이 없는 다중 노드 후보를 거부합니다. 로컬 `dure plan --model auto`의 기존 3×24GB 호환 예외만 계획에 사전 검증 경고를 남기며 이 검사를 건너뜁니다.
...
`verified_at`은 계획의 전체 배정 노드가 모두 검증에 성공하고 각 노드의 Agent가 0.3.12 이상인 경우에만 기록합니다. 일부 노드 또는 Ray head만 검증한 결과와 구 Agent의 성공 결과는 조회 가능하지만 롤백 권한으로 승격하지 않습니다.

롤백 API는 `node_ids`, 엄격한 `apply`, `serve`만 받습니다. 클라이언트가 대상 세대, 계획, 모델 다운로드, 이미지 내려받기, 임의 명령, Docker 옵션, 환경 변수, 마운트나 호스트 경로를 지정할 수 없습니다. 서버는 계보의 최신 소스와 그 소스가 직접 가리키는 `verified_at` 보유 직전 세대를 선택하고 다음을 다시 검사합니다.
...
0.3.12 업그레이드는 controller와 migration을 먼저 적용하고 Agent를 나중에 작은 단위로 진행합니다. migration downgrade는 `PREPARED` 또는 활성 operation, operation에 연결된 `QUEUED`·`RUNNING` task가 있으면 거부합니다. 이 검사를 우회해 task 연결이나 operation 행을 직접 삭제하면 감사와 재시도 펜싱이 깨질 수 있습니다.
