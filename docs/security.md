...
## 모델 레지스트리와 계획된 추천기의 경계

모델 레지스트리의 영속 스키마, 관리자 인증 API, 고정 리비전·다이제스트 검증과 상태 전이는 구현되었습니다. 정책 기반 추천은 아직 구현 중이며 호스트 변경 권한이 아닙니다. 추천은 승인된 최신 인벤토리와 `ACTIVE` 모델 릴리스만 평가하며, 배포 구성 생성과 적용은 별도 운영자 행동입니다.
