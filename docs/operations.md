...
정책 기반 `recommend`, 벤치마크 승격 게이트, 추천 스냅샷 조회와 명시적 수락을 통한 적용 전 배포 세대 생성은 구현되어 있습니다. 세대별 영속 배포 작업(`operation`), 검증 완료 시각(`verified_at`), 검증된 세대로의 명시적 롤백은 아직 구현되지 않았습니다. 추천은 데이터베이스에 저장된 노드 프로필과 `ACTIVE` 모델 릴리스만 평가합니다.
