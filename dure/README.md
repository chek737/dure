...
- [Fleet 후보 생성과 결정론적 스케줄러](docs/fleet-scheduler.md) — exact 증적 기반 다중 배포 조합과 불변 추천 API·CLI, 수락·적용은 후속 범위
...
- Fleet 추천은 PRIMARY·SUPPLEMENTARY exact 증적을 여러 독립 배포 후보로 조합하고 100개 노드 이상도 고정 노드 수 분기 없이 계산합니다. `dure admin fleet recommend/show`와 관리자 API는 전체 평가를 콘텐츠 주소 불변 스냅샷으로 멱등 저장·조회합니다. 현재 Fleet 추천 수락, 원자적 노드·GPU 예약과 다중 세대 적용은 아직 구현되지 않았습니다. 제품 노드 수 상한 대신 후보 512개와 탐색 상태 250,000개의 명시적 안전 한도를 사용하며, 탐색 예산이 소진되면 결정론적 유효 해와 `search_complete=false`를 반환합니다.
