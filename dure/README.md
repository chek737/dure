...
- 이기종 GPU와 대규모 노드 풀에서 Fleet 결정성·비중첩·운영 한도를 검사하는 합성 수용 테스트 매트릭스
...
- 수락된 Fleet의 배포별 준비·적용·API 시작·검증 상태와 실패를 분리해 추적하는 전용 Fleet runtime
...
- [Agent 설정과 credential 회전 운영 절차](docs/agent-operations.md) — `/etc/dure` 설정, HTTPS/TLS, 안전한 credential 회전·재등록
- [단일 GPU 릴리스 자동 활성화](docs/activation.md)
- [릴리스 수용 검증](docs/release-validation.md) — v0.4.14 activation 순서 회귀와 3×24GiB `PP=3` 검증 절차
- [릴리스 증적 기록](docs/release-evidence/README.md) — 절차와 실제 GPU 수용 결과를 분리해 보관하는 형식
- [릴리스 실행 체크리스트](docs/release-runbook.md) — 승인·중단 기준·재시도·APT 게시 후 확인을 한 release record로 연결
- [보안 모델](docs/security.md)
- [네트워크·방화벽 운영 절차](docs/networking.md) — Controller·Agent·Ray/vLLM·NCCL 통신 경계와 포트 검증
- [PostgreSQL 백업·복구·재해 복구](docs/disaster-recovery.md) — backup, restore drill, migration 실패와 credential 복구
- [데이터 보존·격리·삭제 정책](docs/data-retention.md) — DB·audit·evidence·journal·model cache의 보존 기준과 수동 삭제 승인
- [관측·장애 대응 운영 절차](docs/observability.md) — systemd, heartbeat, task, DB 신호와 외부 알림 기준
- [GPU 노드 폐기·교체 운영 절차](docs/node-lifecycle.md) — revoke·unjoin, cache 보존·격리, package 제거와 새 node 재등록
- [관리자·Agent API 계약](docs/api-contract.md) — 인증, 재시도, 오류·민감 정보 처리 규칙
- [지원 매트릭스](docs/support-matrix.md) — OS·패키지·모델·GPU·TP/PP·런타임의 현재 지원 범위
- [버전 호환성과 롤링 업그레이드](docs/compatibility-upgrades.md) — Controller·Agent·migration·backend의 최소 조건과 안전한 rollout·복구
- [용어집](docs/glossary.md) — NCCL, TP, PP, rank, STAGE 등 핵심 용어
...
- [Fleet 후보 생성과 결정론적 스케줄러](docs/fleet-scheduler.md) — exact 증적 기반 다중 배포 조합, 불변 추천·원자적 수락, 명시적 실행과 이기종·대규모 합성 수용 매트릭스
- [모델 아티팩트 매니페스트와 배포 계약](docs/artifact-distribution.md) — `STAGE`·`FULL_SNAPSHOT` 선택, 중앙 캐시 수명 주기와 격리 구현
- [모델 반입·승인 정책](docs/model-onboarding-policy.md) — allowlist, 원본 revision·라이선스·runtime 검토, 승격과 철회 책임
...
- [릴리스 권한과 출처 관리](docs/release-governance.md)
...
### 소스 checkout과 설치 package 구분

이 저장소의 source checkout 버전, 미러에서 설치 가능한 package 버전, 실제 설치된 binary
버전은 서로 다를 수 있습니다. 같은 버전 문자열이나 Git tag만으로 source-to-package 출처가
증명되지는 않습니다.

```bash
# 현재 설치 후보와 실제 설치 version을 확인합니다.
apt-cache policy dure
dure --version
```

package를 운영에 사용하기 전에는 해당 release의 `.deb` SHA-256, provenance record와 APT
`InRelease` 서명을 함께 확인합니다. 현재 배포 권한, 증명 범위와 공식 조직 배포로 전환하기
위한 조건은 [릴리스 권한과 출처 관리](docs/release-governance.md)를 따릅니다.
...
호스트 전체 `apt upgrade`는 별도 유지보수 작업입니다. Docker CE 업그레이드가 Docker daemon을 재시작할 수 있으므로 bootstrap의 재시작 승인과 같은 영향 검토를 거쳐 실행합니다. 현재 Dure APT **미러**는 `amd64` 패키지만 게시합니다. bootstrap 엔진은 Ubuntu 22.04·24.04의 `amd64`와 `arm64`를 지원하지만, `arm64`에서는 Dure CLI·Agent 실행 파일뿐 아니라 패키지와 동일한 `dure-agent.service`와 `/etc/dure/dure-client.env`를 별도 방식으로 먼저 설치해야 합니다.
...
stage export 전용 vLLM 초기화는 `max_model_len=128`, `gpu_memory_utilization=0.95`로 고정해 24 GiB GPU에서 불필요한 장문 KV cache·activation profile을 만들지 않습니다. 이 제한은 export 과정에만 적용되며 artifact tensor와 실제 배포 context 계약에는 영향을 주지 않습니다.
...
rank별 `STAGE` 분산 load는 `scripts/acceptance-vllm-stage-ray-pp.py`로 별도 검증합니다. 이 검사도 기본값은 `NOT_RUN(77)`이며, 신뢰된 2·3노드 환경에서만 각 노드의 서로 다른 stage 캐시가 같은 컨테이너 경로 `/models/model`에 결합되는지, `--load-format sharded_state`로 실제 load되는지, vLLM 0.9.0 V0 `AsyncLLMEngine`의 최소 추론이 성공하는지를 확인합니다. vLLM 0.9.0은 pipeline parallel 생성을 동기 `LLM`에서 거부하므로 수용 검사가 동기 경로로 대체되어서는 안 됩니다.
...
Fleet 추천 생성·조회·수락은 호스트를 변경하지 않습니다. 수락은 중앙 DB에 generation 1 배포와 exact 노드·GPU 예약만 만들며, 다음 명령으로 상태를 확인합니다.

```bash
dure admin fleet status <fleet-id>
```

모델·이미지 준비는 운영자가 전용 명령을 명시적으로 실행한 뒤에만 시작합니다. 이 명령은 저장된 후보와 현재 모델·런타임 레지스트리, exact qualification 증적, 노드·GPU 인벤토리를 다시 검사하고 각 배포에 `PREPARE_MODEL → PREPARE_IMAGE` 작업을 만듭니다.

```bash
dure admin fleet prepare <fleet-id>
```

모든 노드 준비가 성공한 배포만 다음 명시적 명령으로 적용합니다.

```bash
dure admin fleet apply <fleet-id>
```

배포 하나의 실행 순서는 `APPLY(전체 노드) → START_API(head) → VERIFY_API(head) → VERIFY(전체 노드) → ACTIVE`입니다. 한 배포의 준비·적용·검증 실패는 같은 Fleet의 다른 배포 진행을 자동 취소하지 않습니다. 실패한 배포에는 `failure_phase`와 폐쇄형 `failure_code`를 남기며, Dure는 기존 컨테이너 중지, 자동 롤백이나 예약 해제를 수행하지 않습니다. 원인을 조사하는 동안에도 Fleet 예약은 유지됩니다.

## 현재 제한 사항

- Fleet 추천은 PRIMARY·SUPPLEMENTARY exact 증적을 여러 독립 배포 후보로 조합하고 100개 노드 이상도 고정 노드 수 분기 없이 계산합니다. `dure admin fleet recommend/show/accept/status/prepare/apply`와 관리자 API를 제공하며, 수락은 모든 generation 1 배포와 exact 노드·GPU 예약을 한 트랜잭션으로 생성하되 호스트를 변경하지 않습니다. 전용 `prepare`와 `apply`를 각각 명시해야 모델·이미지 준비와 컨테이너 적용·검증이 시작됩니다. 배포별 실패는 격리하지만 자동 재배치·자동 롤백·자동 중지·예약 해제는 제공하지 않습니다. 제품 노드 수 상한 대신 후보 512개와 탐색 상태 250,000개의 명시적 안전 한도를 사용하며, 탐색 예산이 소진되면 결정론적 유효 해와 `search_complete=false`를 반환합니다.
...
- 중앙 제어면은 승인된 단일 노드에서 고정된 네 가지 작업 부하를 `BENCHMARK` 작업으로 실행하고 결과를 증적으로 수집할 수 있습니다. Agent 0.3.26 이상은 명시적 승인으로 HTTPS 중앙 origin의 exact 모델과 digest 이미지를 먼저 준비하고 패키지의 고정 벤치마크 실행기를 사용할 수 있으며, `dure admin activate`는 등록부터 API 검증까지 연결합니다. Hugging Face hub snapshot 직접 실행, 다중 노드 네트워크·NCCL 시험, 전체 동시성 매트릭스와 24시간 복구 검증은 아직 지원하지 않습니다.
...
Fleet 수용 테스트는 4·8·12·24·48·80GiB 합성 노드와 다중 GPU 노드, exact 검증 후보를 사용해 네 모델 allowlist, `TP=1`, 노드당 GPU 한 장, 노드·GPU 비중첩, 입력 순서 독립성, 구조화된 미배정 사유와 제한된 탐색 결과를 회귀 검증합니다. 5노드에서 72B/PP=3과 14B를 함께 선택하고, 6노드에서 비중첩 72B/PP=3 복제본 두 개를 선택하며, 1노드와 이기종 128노드 입력에 노드 수별 제품 분기가 없음을 검사합니다. 이는 한 모델을 임의의 `PP=N`으로 새로 분할한다는 뜻이 아니며, 개별 배포는 현재 runtime과 exact 증적이 허용하는 `PP=1/2/3` 후보만 조합합니다.

제품 노드 수에는 하드 상한이 없지만 기본 `max_candidates=512`, `max_search_states=250000`은 계속 적용됩니다. 후보 한도를 넘으면 명시적으로 거부하고, 탐색 예산을 소진하면 결정론적 유효 결과와 `search_complete=false`를 반환합니다. 기본 테스트는 메모리 내 계산·SQLite·모의 실행기를 사용하므로 실제 GPU·Docker·Ray·vLLM·NCCL 동작, 실제 PostgreSQL의 경합이나 대규모 부하는 별도 환경에서 검증해야 합니다.
