...
- [Fleet 후보 생성과 결정론적 스케줄러](docs/fleet-scheduler.md) — 내부 읽기 전용 평가 구현, 영속 API·수락·적용은 후속 범위
...
- 읽기 전용 Fleet 평가기는 PRIMARY·SUPPLEMENTARY exact 증적을 여러 독립 배포 후보로 조합하고 100개 노드 이상도 고정 노드 수 분기 없이 계산합니다. 다만 현재는 내부 서비스이며 영속 Fleet 추천, 관리자 CLI·API, 원자적 수락·예약과 다중 세대 적용은 아직 구현되지 않았습니다. 제품 노드 수 상한 대신 후보 512개와 탐색 상태 250,000개의 명시적 안전 한도를 사용하며, 탐색 예산이 소진되면 결정론적 유효 해와 `search_complete=false`를 반환합니다.
