...
- 중앙 제어면은 노드 프로필, 관측·희망 상태, 추천·인벤토리 스냅샷, 배포 세대, Fleet 예약·배포별 runtime, 작업, 모델·stage variant 레지스트리, 노드별 아티팩트 캐시와 추가 전용 상태 이벤트, 검증 증적, 자격 증명과 감사 이벤트를 저장합니다.
...
`0.3.16`의 `FULL_SNAPSHOT` 계층과 0.3.19의 rank별 `STAGE` 계층은 같은 중앙 준비 operation·관리자 CLI·Agent 작업을 사용합니다. 0.3.20의 추천기는 정확히 검증된 `STAGE` 후보와 독립된 `FULL_SNAPSHOT` 후보를 함께 평가하지만, 추천과 수락 자체는 여전히 호스트를 바꾸지 않습니다. 운영자가 수락된 세대의 준비 미리보기를 확인하고 같은 요청에 명시적으로 `--apply`를 지정한 뒤에만 `PREPARE_MODEL → PREPARE_IMAGE` 작업이 실행됩니다. GPU 노드 추가만으로 준비를 자동 시작하지 않습니다.
...
| 상태 | 의미와 실행 영향 |
|---|---|
| `READY` | 현재 준비 성공 증적과 전체 identity가 일치해 실행 게이트를 통과할 수 있습니다. |
| `STALE` | variant 철회, identity 불일치 또는 `READY` 캐시의 격리 요청 때문에 다시 준비해야 합니다. |
| `MISSING` | 완전한 probe에서 기존 중앙 캐시가 사라진 것으로 관찰됐습니다. |
| `CORRUPT` | 완전한 probe의 손상·위험 관찰 또는 실행 검증 실패가 기록됐습니다. |
| `QUARANTINED` | 운영자가 명시적으로 캐시를 보존 격리 영역으로 옮겼습니다. |

완전한 폐쇄형 probe만 이미 중앙에 알려진 캐시를 `STALE`·`MISSING`·`CORRUPT`로 강등할 수 있습니다. `complete=false`인 검사와 구 Agent의 legacy profile은 아무 상태도 바꾸지 않으며, 완전한 probe에서 정상 항목을 관찰해도 `READY`로 승격하지 않습니다. `READY` 복구에는 새 현재 준비 작업의 성공이 필요합니다.

격리는 자동 삭제나 자동 퇴출이 아닙니다. 읽기 전용 조회·검증 뒤 운영자가 명시적으로 적용해야 하며, 참조가 없는 정확한 캐시 디렉터리를 `/var/lib/dure/models/.dure-quarantine` 아래로 덮어쓰기 없는 원자적 이동으로 보존합니다. 활성 준비·배포·벤치마크 작업, 활성 operation, 현재 세대와 검증된 직접 롤백 선행 세대가 참조하면 격리를 거부합니다. 공유 CAS 청크를 자동 삭제하거나 노드 간 P2P로 전송하는 기능도 없습니다.
...
0.3.20 추천기는 현재 source manifest·runtime·토폴로지와 정확히 일치하는 모든 `VALIDATED` variant를 독립 후보로 만들고 artifact-set digest 순으로 결정론적으로 평가합니다. 같은 품질·모델이면 실행 가능한 `STAGE` 후보를 `FULL_SNAPSHOT` 후보보다 먼저 평가하지만, 디스크나 rank 계약을 통과하지 못한 `STAGE`를 준비 중에 임의로 `FULL_SNAPSHOT`으로 바꾸지는 않습니다. `FULL_SNAPSHOT`은 자체 디스크 조건을 가진 별도 후보입니다.

추천 수락으로 만든 불변 세대는 캐시 종류, source·rank 매니페스트, variant·loader 계약, 실행 backend, 노드 UUID→rank 결합, 네트워크와 stage 검증 증적을 고정합니다. 이후 준비 요청은 이 저장 선택을 그대로 소비하며 선택 종류나 variant를 덮어쓸 수 없습니다. Agent는 계약 전체의 복합 identity로 자신의 rank 캐시만 원자적으로 활성화하고, 런타임은 시작 직전에 전체 파일·marker를 재해시한 뒤 서로 다른 host 경로를 동일한 `/models/model`에 읽기 전용 mount합니다. vLLM 명령에는 고정 `--load-format sharded_state`가 추가됩니다. 상세 계약은 [stage artifact 문서](stage-artifacts.md)를 따릅니다.
...
VLLM_RAY_PP_V1 / vLLM 0.9.0 / V0 / Ray 계획
        ↓ 모든 노드의 exact READY 캐시와 최신 digest 고정 이미지 준비 증적 확인
...
계획·프로필·캐시·이미지 identity와 고정 포트 **값**은 Docker 변경 전에 검사합니다. 0.3.20부터 일반 적용·시작·재시작·검증과 롤백 대상 시작은 노드별 exact `READY` 캐시와 현재 성공 준비 시도, 최신 이미지 준비 증적을 함께 요구합니다. 캐시가 `STALE`·`MISSING`·`CORRUPT`·`QUARANTINED`이거나 준비 증적이 과거 시도에 속하면 호스트 변경 전에 실패합니다.

다만 현재 구현은 host의 `6379`, `20000-21000`, `8000` 점유 여부를 별도 preflight하지 않습니다. 다른 프로세스나 이전 세대가 포트를 사용하면 컨테이너 시작 뒤 restart/readiness 실패로 발견합니다. 이 경우 operation을 실패 상태로 보존하고 정확한 deployment·generation·node·rank label의 컨테이너만 `STOP`으로 정리한 뒤 원인을 해결해 제한적으로 재시도하거나 검증된 직전 세대로 명시적으로 롤백합니다. 롤백은 `STOP_SOURCE`가 모두 성공한 직후 대상의 exact `READY` 캐시와 최신 이미지 증적을 다시 잠금 검사합니다. 이 사이 상태가 바뀌었다면 `START_TARGET` 작업, 다운로드와 image pull을 만들지 않고 operation을 실패로 보존하므로, 캐시를 다시 준비한 뒤 명시적으로 재시도해야 합니다. 동일 GPU의 전환이 이미 시작됐다면 이전 컨테이너가 계속 실행된다고 보장할 수 없습니다. Dure는 host NVIDIA driver를 자동 설치·교체하지 않으며 driver·CUDA·하드웨어 문제는 운영자가 고정 런타임 호환표에 따라 해결해야 합니다.
...
0.4.1은 Fleet 허용 목록의 Qwen2.5 AWQ 7B·14B·32B·72B에 한해 버전이 고정된 자동 배치 프로필 생성기를 추가합니다. 7B·14B·32B는 `TP=1/PP=1`, 72B는 `TP=1/PP=1·2·3`의 `DRAFT`만 만듭니다. 프로필에는 노드 수, rank별 최소 VRAM·디스크, 컨텍스트·동시성, 네트워크·NCCL와 SLO 임계값, 정규 spec digest를 고정합니다. 미리보기는 DB를 바꾸지 않고 명시적 `apply=true`만 한 트랜잭션으로 누락 프로필을 만듭니다. 이 단계에서는 DRAFT를 검증 또는 추천 가능 상태로 올리지 않으며 호스트 작업도 만들지 않습니다.

0.4.4는 단일 배포용 `deployment_recommendations`와 분리된 `fleet_recommendations`를 추가합니다. Fleet 한 행은 요청 정책, 전체 GPU 풀, exact 증적 기반 후보, 서로 겹치지 않는 선택 배포 전체, 탈락 대안과 미배정 노드를 한 콘텐츠 주소 스냅샷으로 저장합니다. 생성 시각은 ID에 포함하지 않으며 동일 관측·정책은 같은 ID를 재사용합니다. 추천 생성·조회는 이 기록 외에 deployment, task, 예약 또는 호스트 상태를 만들지 않습니다. Fleet 수락과 여러 세대 생성은 아직 이 단계의 권한이 아닙니다.

0.4.5는 `fleets`와 `fleet_resource_reservations`를 추가합니다. 수락 경계는 저장 추천을 잠금 안에서 정확히 재평가하고, 선택된 후보마다 독립 generation 1 배포를 만든 뒤 exact 노드 UUID·GPU index·GPU UUID·rank 예약을 연결합니다. Fleet 안과 전체 활성 Fleet 사이에서 노드·GPU 중복을 DB 제약과 서비스 검사로 함께 금지하며 하나라도 실패하면 전체 트랜잭션을 롤백합니다. 이 단계의 수락은 `CREATED` 중앙 기록과 예약만 만들고 Agent task나 호스트 변경은 만들지 않습니다.

0.4.6은 배포마다 `fleet_deployment_runtime` 행을 만들고 전용 `fleet status`, `fleet prepare`, `fleet apply` 경계를 추가합니다. 추천·조회·수락·상태 조회는 호스트를 변경하지 않습니다. `prepare`는 저장 후보의 현재 레지스트리·인벤토리·exact qualification 증적·캐시 identity를 다시 검사한 뒤 각 배포의 `PREPARE_MODEL → PREPARE_IMAGE`만 큐잉합니다. `apply`는 준비된 배포마다 하나의 current operation을 고정하고 `APPLY(전체 노드) → START_API(head) → VERIFY_API(head) → VERIFY(전체 노드)`를 진행합니다. 모든 검증이 성공한 배포만 `ACTIVE`가 됩니다.

Fleet runtime은 배포별 실패를 격리합니다. 한 배포가 `PREPARE_FAILED`, `APPLY_FAILED`, `VERIFY_FAILED`가 되어도 다른 배포의 반복문과 이미 큐잉된 작업을 자동 취소하지 않습니다. 실패는 Fleet 전체에 `PARTIAL_FAILED` 또는 `FAILED`로 투영하지만 컨테이너 중지, 자동 롤백, 자동 재배치와 노드·GPU 예약 해제를 수행하지 않습니다.
...
신선한 인벤토리 스냅샷
        ↓
결정론적 계획기와 ACTIVE 모델 릴리스의 exact STAGE·독립 FULL 후보 평가
        ↓
추천·정규화 인벤토리 스냅샷 저장 (호스트 변경 없음)
        ↓
운영자 수락 전 현재 상태 재검사
        ↓
캐시·매니페스트·variant·loader·backend·rank·증적이 고정된 불변 배포 세대 생성 (작업 없음)
        ↓
...
Fleet는 여러 세대를 하나의 원자 적용으로 가장하지 않습니다. 수락만 전체 배포·예약을 한 트랜잭션으로 만들고, 이후 실행은 배포별로 격리합니다.

```text
Fleet 추천·수락 ── 호스트 변경 없음
        ↓ 운영자의 fleet prepare
배포별 PREPARE_MODEL → PREPARE_IMAGE
        ↓ 운영자의 fleet apply
배포별 APPLY → START_API → VERIFY_API → VERIFY
        ↓
배포별 ACTIVE 또는 단계별 실패, 예약은 유지
```
...
롤백 task는 `accept_model_download=false`, `pull_image=false`를 강제합니다. 대상 모델 캐시는 중앙 상태가 exact `READY`여야 하고 최신 다이제스트 이미지 준비 증적도 모든 노드에 미리 있어야 합니다. 이 조건은 롤백 적용 전과 `STOP_SOURCE` 성공 직후 `START_TARGET`을 만들기 전에 다시 검사합니다. 현재 구현은 같은 GPU에서 소스 컨테이너를 중지하고 대상 세대를 다시 만드는 방식이므로 중단 시간이 발생할 수 있고 블루·그린 배포가 아닙니다.
...
`PREPARE_MODEL`과 `PREPARE_IMAGE`는 일반 작업 생성 API로 만들 수 없습니다. 추천을 수락해 만든 세대에 대해 관리자 전용 deployment 준비 API가 먼저 저장된 캐시 선택의 불변 preview를 만들고, 같은 요청에 명시적으로 `apply=true`를 지정한 뒤에만 노드별 모델 작업을 만듭니다. 모델 해시와 marker 검증이 성공한 노드에만 다이제스트 고정 이미지 작업을 이어서 만들며, 실패 시에는 성공한 단계를 보존하고 실패한 현재 단계만 새 시도 번호로 재시도합니다. 현재 모델 준비 성공과 중앙 `READY` 전이는 같은 트랜잭션에서 기록되고, 모든 노드의 모델·이미지 단계가 성공해야 일반 apply가 해당 경로를 소비할 수 있습니다. 롤백은 과거의 정확한 성공 증적과 로컬 캐시·이미지만 사용하고 새 준비 작업이나 네트워크 복구를 만들지 않습니다.

Fleet 소속 세대는 위 단일 배포 API를 직접 사용할 수 없습니다. 빈 본문만 받는 Fleet 전용 prepare API가 배포별 runtime capability를 내부적으로 전달할 때만 기존의 폐쇄형 준비 서비스를 사용할 수 있습니다. Fleet apply도 저장 노드 전체와 `serve=true`인 전용 요청만 허용하고, 실제 첫 `APPLY` task에는 `serve=false`, 다운로드와 image pull 금지를 고정한 뒤 head API 단계와 전체 검증 단계를 서버가 이어서 만듭니다. 클라이언트가 runtime ID, task 종류, 노드 부분집합이나 실행 옵션을 지정할 수 없습니다.

Fleet generation 1은 전용 runtime의 루트이므로 일반 단일 배포 추천의 `previous_generation_id`로 사용할 수 없습니다. 수락 서비스는 추천 재평가 전과 계보 행 잠금 후에 이 경계를 모두 검사하고 `FLEET_LINEAGE_EXTENSION_FORBIDDEN`으로 거부합니다. 따라서 Fleet runtime에 보이지 않는 일반 generation 2가 같은 계보와 operation 잠금을 공유하는 우회 경로는 허용되지 않습니다.

`QUARANTINE_ARTIFACT_CACHE`도 전용 캐시 API만 만들 수 있습니다. 기본 요청은 중앙 상태와 모든 차단 참조를 읽는 미리보기이며 작업 수는 0개입니다. `apply=true`가 있고 중앙이 활성 작업·operation·현재 세대·검증된 직접 롤백 선행 세대의 참조가 없음을 완전히 증명한 경우에만 Agent 0.3.20 이상 노드에 정확히 한 작업을 만듭니다. Agent는 허용된 캐시 종류와 identity의 고정 경로만 보존 영역으로 원자적 이동하며 파일을 삭제하지 않습니다.

`UNJOIN_NODE`는 전용 관리자 명령이 승인된 GPU 노드에만 생성합니다. Agent는 로컬 상태의 exact 배포·세대·노드 label과 일치하는 Dure 컨테이너만 중지하고 성공 결과가 중앙에 기록된 뒤 Agent를 비활성화하고 credential을 제거합니다. 중앙은 성공과 credential 폐기를 같은 트랜잭션에서 기록하며, 다중 노드 배포의 빈 자리를 자동으로 다른 노드에 재배정하지 않습니다.
...
계획은 서버가 발급한 노드 UUID를 사용합니다. 레거시 호스트명 배정은 승인된 노드 하나로만 해석될 때 정규화할 수 있습니다. `VLLM_RAY_PP_V1`은 이 호환 정규화를 허용하지 않고 계획의 UUID 전체 집합을 서버 노드와 직접 대조하며, 비중지 작업은 전체 배정 노드와 backend별 최소 Agent 버전을 요구합니다. `STAGE` 준비·실행은 Agent 0.3.19 이상, 전용 캐시 격리 작업은 Agent 0.3.20 이상을 요구합니다. 중앙 배포 이미지는 OCI 다이제스트로 고정돼야 합니다.
