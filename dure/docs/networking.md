...
5. vLLM API는 head의 loopback `127.0.0.1:8000`에만 bind한다. 외부 inference endpoint가 필요하면 별도 인증·rate limit·감사 설계를 거친 proxy를 추가하며, Dure Control Plane API와 같은 공개 endpoint로 합치지 않는다.
