...
- 실행 직전 선택 GPU compute process를 조회하고, 실행 중 process나 상위 GPU를 확인할 수 없는 MIG process가 있으면 거부합니다. 이 조회와 Docker 시작 사이의 경주는 남으므로 벤치마크 노드에서 외부 GPU 스케줄러를 병행하지 않아야 합니다. 컨테이너는 정상 GPU 한 장만 UUID로 할당하고 네트워크 없음, 읽기 전용 루트·모델·진입점, capability 제거, 권한 상승 금지, `restart=no`와 정확한 벤치마크 레이블을 사용합니다. root 전용 Dure 모델 캐시를 읽기 위해 컨테이너 기본 사용자는 root이지만 host 쓰기 mount와 Linux capability는 주지 않습니다. RAM은 전체·가용량 중 작은 값의 절반과 32GiB 중 작은 값으로 제한하고 8GiB 미만이면 거부하며, swap 상한은 RAM과 같고 CPU quota는 논리 CPU의 절반·최대 8코어입니다. stdout·stderr는 실행 중 합계 64KiB를 넘으면 작업을 중단합니다.
- 기존 작업 부하나 다른 Dure 벤치마크 컨테이너가 감지되면 벤치마크를 거부합니다. 재시도는 폐쇄형 payload와 대상 노드 조건을 확인한 직후, 현재 빌드·캐시·프로필·가용 자원·이미지보다 먼저 정확한 작업 컨테이너를 조정하고 그 뒤 프로필을 새로 조사합니다. Docker 시작 시각에서 고정 실행 예산 240초와 정리 여유 60초를 더한 총 300초를 넘긴 정확한 활성 컨테이너 또는 같은 UUID의 `created`·`exited`·`dead` 컨테이너만 레이블을 다시 확인한 뒤 중지·제거합니다. 240초는 최소 측정 시간이 아니며 예열 2회와 고정 20회 요청이 끝나면 즉시 실제 경과 시간을 보고합니다. 모델·이미지 준비 시간은 이 상한 밖입니다. inspect·부재 확인·stop·remove가 불확실하면 terminal 결과를 보내지 않고 임대 재시도를 기다립니다. 배포 컨테이너와 다른 벤치마크는 중지하거나 제거하지 않습니다.
- 결과와 실패 경계는 고정 스키마·실패 코드만 허용합니다. 프롬프트, 로그, stdout·stderr와 예외 원문은 중앙 작업 결과나 감사 이벤트에 저장하지 않습니다.

현재 자동 벤치마크는 승인된 신뢰 노드의 단일 GPU 작업만 실행합니다. 요청의 Dure 커밋은 공식 Debian 패키지의 읽기 전용 빌드 metadata 또는 명시적인 개발 환경 값과 대조하고, 완료 이력 재전송에도 같은 값을 요구합니다. 외부에서 등록한 다중 노드 증적을 추천에 사용할 수는 있지만, Dure가 다중 노드 네트워크·NCCL 시험이나 GPU stage 수용 harness를 자동 실행하지는 않습니다. 전체 작업 부하 매트릭스 또는 24시간 복구 검증도 증명하지 않으며, Agent 결과의 암호학적 서명과 원본 provenance도 아직 없습니다. 관리자·Agent API, 패키지의 벤치마크 진입점과 digest 고정 vLLM 이미지는 계속 신뢰된 운영자 경계 안에서만 사용해야 합니다. 포트·사설망·NCCL interface 검증은 [네트워크·방화벽 운영 절차](networking.md), Agent 주소·TLS·credential 회전은 [Agent 설정과 credential 회전 운영 절차](agent-operations.md), DB 복구는 [PostgreSQL 백업·복구·재해 복구](disaster-recovery.md), 상태 확인·redaction은 [관측·장애 대응 운영 절차](observability.md), HTTP 호출의 인증·오류 규칙은 [관리자·Agent API 계약](api-contract.md)을 따릅니다.
