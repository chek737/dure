...
0.3.26 요구 사항, artifact origin과 재시도 절차는
...
모델 아티팩트와 digest 고정 런타임을 결합한 `DRAFT` 모델 릴리스에만 자동 프로필을 만들 수 있습니다. 대상 모델은 Qwen2.5 AWQ 7B·14B·32B·72B 네 개로 닫혀 있고 결과는 모두 `TP=1`입니다. 기본 요청은 변경 없는 미리보기입니다.
...
curl -sS -X POST \
  ...
  -d '{"apply":false}' \
  https://<control-plane>/v1/admin/model-releases/<release-id>/placements/generate
```

응답의 각 프로필에서 모델 ID, `generator_version`, TP/PP, 최소 VRAM·디스크, 컨텍스트·동시성, 네트워크 임계값과 `spec_digest`를 검토합니다. `state=MISSING`은 아직 없고 `state=EXISTS`는 동일 digest로 이미 저장된 프로필입니다. 명시적 적용만 DB에 저장합니다.
...
curl -sS -X POST \
  -H "Authorization: Bearer <admin-token>" \
  -H "Content-Type: application/json" \
  -d '{"request_id":"<request-uuid>","placement_id":"<placement-uuid>","node_ids":["<node-uuid>"],"apply":false}' \
  https://<control-plane>/v1/admin/profile-qualifications/prepare
```
...
기본 개별 BENCHMARK 적용은 `/var/lib/dure/models` 아래의 완전히 펼쳐진 모델과 정확한 `.dure-model.json` metadata, 레지스트리에 고정된 로컬 OCI 이미지를 요구합니다. Agent 0.3.26 이상에서 apply 본문에 `prepare_model=true`, `pull_image=true`를 명시하거나 `dure admin activate --apply`를 사용하면 신뢰 artifact origin에서 exact 모델을 준비하고 digest 이미지를 받은 뒤 실행합니다. 패키지의 고정 `dure-benchmark`가 이미지에 읽기 전용으로 마운트되며 이미지는 vLLM·CUDA 환경만 제공합니다. Hugging Face hub snapshot의 외부 blob 링크를 직접 실행 경로로 사용하지 않습니다. 다른 활성 LLM 작업 부하·Dure 벤치마크 컨테이너·선택 GPU compute process가 있거나 캐시·프로필 지문·이미지가 다르면 실패합니다. MIG process는 상위 GPU를 판별하지 않고 모두 거부합니다. 가장 큰 정상 GPU 한 장만 UUID로 할당하며, 그 GPU의 compute capability가 런타임의 `gpu_architectures` 폐쇄 목록과 일치해야 합니다. capability가 없거나 미지원이면 준비·적용·증적 등록·승격을 거부합니다.

컨테이너는 `restart=no`로 실행하며 RAM은 현재 전체·가용량 중 작은 값의 절반(최대 32GiB, 계산값이 8GiB 미만이면 거부), swap은 같은 상한, CPU는 논리 CPU의 절반(최대 8코어)으로 제한합니다. root filesystem은 읽기 전용이고 Triton JIT가 생성한 CUDA 공유 라이브러리를 로드할 수 있도록 `/tmp`만 1GiB의 `exec,nosuid,nodev` tmpfs로 제공합니다. 실행 출력은 합계 64KiB로 제한합니다. 재시도 시 폐쇄형 payload와 정확한 대상 노드를 확인한 직후, 현재 Agent 빌드·캐시·프로필·가용 자원·이미지보다 먼저 같은 작업의 컨테이너를 조정합니다. Docker `StartedAt` 기준 고정 실행 예산 240초에 초기화·정리 여유 60초를 더한 총 300초를 넘긴 경우에만 레이블을 재확인해 중지·제거하고, 실제 컨테이너 실행에도 같은 상한을 사용합니다. 240초는 최소 측정 시간이 아니며 예열 2회와 측정 20회가 끝나면 즉시 실제 경과 시간을 보고합니다. 모델·이미지 준비 시간은 이 컨테이너 상한에 포함되지 않습니다. inspect·stop·remove나 부재 확인이 불확실하면 작업을 실패로 닫지 않고 다음 임대까지 유예합니다. 같은 요청 ID는 기존 실행을 재사용하고, 새 측정은 새 요청 ID로 준비합니다.
...
0003 마이그레이션은 과거 버전에서 증적 없이 `ACTIVE`가 된 릴리스를 `VALIDATED`로 되돌립니다. 0004는 준비된 벤치마크 실행과 작업·증적 연결을 추가하고, 새 지문에서는 가용 메모리·남은 디스크·현재 작업처럼 순간적인 값을 제외합니다. 0005는 불변 추천 스냅샷과 배포 세대 계보를 추가하고 기존 수동 배포는 `lineage_id=id`로 보정하되 과거 generation과 plan JSON은 변경하지 않습니다. 0006은 배포 operation, 노드별 단계·시도, task 연결과 `verified_at`을 추가합니다. 0008은 배포별 아티팩트 준비, 노드 단계와 시도·task 결합을 추가합니다. 0009는 stage artifact variant, rank별 매니페스트와 검증 증적을 추가합니다. 0010은 노드별 exact cache state와 append-only 상태 event를 추가하며 과거 준비 성공을 추측해 `READY`로 backfill하지 않습니다. 0011은 기존 배치 프로필을 `MANUAL/ACTIVE`로 보존하고 컨텍스트·동시성, origin, 독립 상태와 spec digest를 추가합니다. 0012는 profile qualification run·증적·정규화 rank·노드·GPU binding과 프로필의 qualification 참조를 추가하며 기존 수동 프로필을 자동 qualification 대상으로 바꾸지 않습니다. 0012 업그레이드는 스키마를 바꾸기 전에 기존 `AUTO` 프로필이 모두 `DRAFT`인지 검사하고, 하나라도 다른 상태이면 전체 업그레이드를 거부합니다. 이 경우 먼저 DB 백업을 확인하고 해당 프로필을 현재 지원되는 수명 주기로 정리한 뒤 유지보수 시간에 다시 실행해야 하며, 마이그레이션이 상태를 추측해 보정하지 않습니다. 자동 또는 비활성 프로필이 있으면 정보 손실을 피하기 위해 0011 downgrade를 거부합니다. 0003에서 저장한 전체 프로필 지문은 당시 저장 프로필이 그대로인 동안 승격 검사에서 계속 인정합니다. 업그레이드 뒤 기존 릴리스의 모든 활성 배치 프로필에 현재 증적을 등록하고 명시적으로 다시 승격해야 합니다. 이 상태 보정과 스키마 추가는 실행 중인 컨테이너에는 손대지 않습니다.
