...
- 3대 이상의 24GB GPU 노드를 모두 이용한 탄력적 Qwen2.5-72B-AWQ pipeline 계획
- 노드 수명 주기 상태의 원자적 저장
...
- 단일 또는 전체 GPU 노드의 안전한 unjoin과 credential 폐기
- Codex를 이용한 읽기 전용 용량 진단
- 현재 보이는 GPU 수를 자동 감지하는 CUDA/NCCL pipeline overlap 실험
- 기본 모의 실행과 명시적 변경 플래그

GPU pipeline 실험은 `CUDA_VISIBLE_DEVICES`를 반영해 감지된 GPU마다 worker 하나를 실행합니다.

```bash
python3 dure/speed/async_pipeline_optimizer.py

# 일부 GPU만 사용할 때
CUDA_VISIBLE_DEVICES=0,1,2 python3 dure/speed/async_pipeline_optimizer.py
```

이 코드는 독립 벤치마크이며 현재 `dure-agent`나 vLLM serving 경로를 자동 변경하지 않습니다.

## 문서

- [문서 색인](dure/docs/README.md)
- [아키텍처](dure/docs/architecture.md)
- [운영 절차](dure/docs/operations.md)
- [보안 모델](dure/docs/security.md)
- [모델 선택 정책](dure/docs/model-selection.md) — 계획됨
- [벤치마크 및 모델 자격 검증](dure/docs/benchmarking.md) — 계획됨
- [개발·릴리스 절차](dure/docs/development.md)
- [APT 배포](dure/docs/apt-distribution.md)
- [개발 로드맵](dure/docs/roadmap.md)
- [제품 제안서](dure/docs/dure-proposal.md)
...
새 GPU 노드를 설치·승인한 뒤에는 중앙 인벤토리를 갱신해 모든 승인된 온라인 GPU를 포함하는
replacement plan을 만듭니다. 실행 중인 n-stage vLLM에 worker를 즉시 삽입하지 않고,
n+1-stage 계획을 검토한 뒤 별도로 적용합니다.
...
`--nodes <node-id> ...`로 일부 승인 노드만 선택할 수도 있습니다.
...
- 현재 `--model auto`는 고정된 Qwen2.5 AWQ 카탈로그를 사용합니다. GPU 추가에 따른 정책 기반 추천·모델 레지스트리·자동 후보 재계산은 [모델 선택 정책](dure/docs/model-selection.md)에서 계획 중인 기능입니다.
- 적용 모드는 Docker만 지원하며, 물리 노드당 GPU 한 장만 배정합니다.
- 실행 중인 vLLM pipeline의 live resize는 지원하지 않습니다. GPU 추가 시 replacement
  deployment를 검증한 뒤 운영자가 명시적으로 전환해야 합니다.
...
현재 머신 한 대를 직접 분리하려면 해당 GPU 노드에서 실행합니다. 현재 state에 기록된 정확한
deployment label의 Dure 컨테이너만 중지하고, 중앙 credential을 폐기한 뒤 agent를
비활성화합니다. 모델 파일과 Dure가 소유하지 않은 컨테이너는 삭제하지 않습니다.

```bash
sudo dure unjoin
```

관리자는 한 GPU 노드 또는 승인된 모든 GPU 노드에 비동기 unjoin 작업을 보낼 수 있습니다.
CPU utility 노드는 `--all` 대상에서 제외됩니다.
...
unjoin 작업은 GPU 컨테이너 정리에 성공한 뒤에만 credential을 폐기합니다. 단일 pipeline
worker를 제외하면 기존 분산 배포가 불완전해질 수 있으므로 남은 GPU로 replacement
deployment를 생성·검증해야 합니다. unjoin한 머신이 다시 `sudo dure join`을 실행하면 기존
install ID와 node UUID를 재사용해 pending 상태로 등록되며 관리자 승인을 다시 받아야 합니다.

중앙 작업은 `PROBE`, `VERIFY`, `APPLY_DEPLOYMENT`, `START_DEPLOYMENT`, `STOP_DEPLOYMENT`, `RESTART_DEPLOYMENT`, `UNJOIN_NODE`로 고정됩니다. 임의 remote shell 명령은 받지 않습니다.
