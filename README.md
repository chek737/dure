...
- 현재 `--model auto`는 버전이 있는 Qwen2.5 AWQ 정적 카탈로그와 결정론적 선택기를 사용합니다. 중앙 모델 레지스트리, 저장 인벤토리 기반 읽기 전용 추천과 벤치마크 증적 기반 `ACTIVE` 승격 게이트를 제공합니다. 추천 승인과 배포 세대 전환은 [모델 선택 정책](docs/model-selection.md)의 후속 범위입니다. 선택기의 기본 정책과 중앙 추천은 네트워크·NCCL 증적이 없는 다중 노드 후보를 거부하지만, 로컬 `dure plan --model auto`는 기존 3×24GB 계획 호환성을 위해 이 검사만 예외로 두고 계획에 검증 경고를 남깁니다.
...
추천은 배포나 작업을 저장하지 않고 모델 다운로드, 이미지 내려받기, Docker 실행을 유발하지 않습니다. 벤치마크 증적 저장과 승격 게이트는 구현되어 있지만 추천기가 해당 증적을 노드별 네트워크·NCCL 자격으로 연결하지는 않으므로, 중앙 다중 노드 후보는 현재 실패 안전 방식으로 거부됩니다.
