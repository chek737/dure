...
- 현재 `--model auto`는 버전이 있는 Qwen2.5 AWQ 정적 카탈로그와 결정론적 선택기를 사용합니다. 중앙 모델 레지스트리는 제공하지만 GPU 추가에 따른 중앙 정책 기반 추천·벤치마크 증적 기반 승격·추천 승인은 [모델 선택 정책](docs/model-selection.md)에서 계속 구현 중입니다.
