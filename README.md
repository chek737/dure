...
- 현재 `--model auto`는 버전이 있는 Qwen2.5 AWQ 정적 카탈로그와 결정론적 선택기를 사용합니다. 선택기의 기본 정책과 중앙 추천은 네트워크·NCCL 증적이 없는 다중 노드 후보를 거부하지만, 로컬 `dure plan --model auto`는 기존 3×24GB 계획 호환성을 위해 이 검사만 예외로 두고 계획에 검증 경고를 남깁니다. 중앙 모델 레지스트리, 벤치마크 증적 기반 `ACTIVE` 승격 게이트, 추천 스냅샷과 명시적 수락, 세대별 적용·검증 상태와 명시적 롤백도 제공합니다.
...
중앙 작업은 `PROBE`, `BENCHMARK`, `VERIFY`, `APPLY_DEPLOYMENT`, `START_DEPLOYMENT`, `STOP_DEPLOYMENT`, `RESTART_DEPLOYMENT`로 고정됩니다. `BENCHMARK`는 전용 준비·적용 API로만 만들 수 있고 임의 remote shell 명령은 받지 않습니다.
...
다중 노드 후보는 네트워크·NCCL 증적 기능이 추가되기 전까지 추천에서 실패 안전 방식으로 거부됩니다. 현재 세대 검증과 롤백도 네트워크·NCCL 시험이나 24시간 복구 검증을 대신하지 않습니다.
