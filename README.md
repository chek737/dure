...
- [모델 아티팩트 매니페스트와 배포 계약](docs/artifact-distribution.md) — 부분 구현
...
새 콘텐츠 주소 캐시는 Dure가 소유한 고정 루트만 사용하고 task가 경로나 raw URL을 지정할 수 없게 설계되었습니다. 패키지는 `/var/lib/dure`를 root 소유 경계로 두고 중앙 서버용 쓰기 경로를 `/var/lib/dure/server`로 분리합니다. 캐시 부모가 symlink이거나 다른 사용자에게 쓰기 가능하면 준비·인벤토리 신뢰·벤치마크 사용을 실패 안전 방식으로 거부합니다.
...
- 롤백은 최신 세대의 검증된 직전 세대로만 가능하며, 전체 배정 노드·동일 토폴로지·승인·온라인 상태·0.3.12 이상 Agent와 다이제스트 고정 이미지를 요구합니다. 같은 GPU에서 이전 세대 컨테이너를 다시 만드는 절차이므로 서비스 중단이 생길 수 있으며 블루·그린 전환이 아닙니다.
...
- 중앙 아티팩트 매니페스트 등록·조회와 노드 내부의 콘텐츠 주소 기반 `FULL_SNAPSHOT` 준비 라이브러리는 구현되었습니다. 준비기는 중단 다운로드와 파일 조립을 digest별 고정 부분 파일에서 재개하고 전체 청크·파일 해시, `config.json` 양자화 일치, 소유권·경로·디스크를 검사한 뒤 marker를 마지막에 기록합니다. 그러나 이 라이브러리를 호출하는 중앙 준비 API·CLI와 Agent 작업, OCI 이미지 준비는 아직 없으므로 추천이나 수락이 자동 다운로드를 시작하지 않습니다.
- 오염된 CAS·staging·final은 덮어쓰거나 자동 삭제하지 않고 실패합니다. 현재 공식 quarantine·캐시 퇴출 및 전역 참조 검사 명령이 없습니다. 특히 공유 CAS 청크는 모든 매니페스트와 진행 중 준비의 미참조를 증명할 수 없으면 옮기거나 지우면 안 됩니다. `STAGE` 캐시는 식별할 수 있지만 준비·실행하지 않습니다.
...
- 정규화된 모델 아티팩트 매니페스트 등록은 제공하지만 SHA-256은 게시자 서명이나 신뢰 provenance가 아닙니다. 게시자·이미지 서명 검증은 계획 단계입니다.
...
중앙 작업은 `PROBE`, `BENCHMARK`, `VERIFY`, `APPLY_DEPLOYMENT`, `START_DEPLOYMENT`, `STOP_DEPLOYMENT`, `RESTART_DEPLOYMENT`로 고정됩니다. `BENCHMARK`는 전용 준비·적용 API로만 만들 수 있고 임의 remote shell 명령은 받지 않습니다. `PREPARE_MODEL`과 `PREPARE_IMAGE`는 현재 작업 열거형·API·CLI에 없으며 다음 중앙 준비 PR에서만 추가합니다.
...
`recommend`는 추천과 정규화된 인벤토리 스냅샷 한 건만 멱등하게 저장합니다. `accept`는 저장 당시와 현재의 추천 ID·카탈로그·정책·인벤토리 지문·선택 결과가 모두 같을 때만 `CREATED` 배포 세대 한 건을 만들며, 같은 요청의 재시도는 기존 세대를 반환합니다. 이전 세대를 지정하면 해당 계보의 최신 세대에만 연결할 수 있습니다.

추천 생성과 수락은 `PROBE`나 배포 작업을 만들지 않고, 모델 다운로드, 이미지 내려받기, Docker 실행, 기존 컨테이너 중지를 유발하지 않습니다. 실제 호스트 변경에는 별도의 `dure admin apply`가 필요합니다. 전체 배정 노드의 `VERIFY`가 성공하고 모든 노드의 Agent가 0.3.12 이상일 때만 해당 세대의 `verified_at`이 롤백 증거로 기록됩니다.

롤백 준비도 작업을 만들지 않습니다. `--apply`를 명시한 뒤에만 `STOP_SOURCE → START_TARGET → VERIFY_TARGET` 순서로 진행하며, `--serve`를 선택하면 Ray head에서 `START_API → VERIFY_API`를 이어서 실행합니다. 한 단계의 모든 노드가 성공해야 다음 단계로 넘어가고, 실패 뒤 같은 명령에 `--apply`를 다시 지정하면 현재 단계의 실패 노드만 새 시도 번호로 재시도합니다. 롤백은 모델이나 이미지를 다운로드하거나 내려받지 않습니다.
