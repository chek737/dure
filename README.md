...
- 현재 `--model auto`는 버전이 있는 Qwen2.5 AWQ 정적 카탈로그와 결정론적 선택기를 사용합니다. 선택기의 기본 정책과 중앙 추천은 네트워크·NCCL 증적이 없는 다중 노드 후보를 거부하지만, 로컬 `dure plan --model auto`는 기존 3×24GB 계획 호환성을 위해 이 검사만 예외로 두고 계획에 검증 경고를 남깁니다. 중앙 모델 레지스트리, 벤치마크 증적 기반 `ACTIVE` 승격 게이트, 추천 스냅샷과 명시적 수락을 통한 배포 세대 생성도 제공합니다. 세대에 결합된 배포 작업 상태 추적, 검증 완료 기록과 명시적 롤백은 [모델 선택 정책](docs/model-selection.md)의 후속 범위입니다.
...
추천 생성과 수락은 `PROBE`나 배포 작업을 만들지 않고, 모델 다운로드, 이미지 내려받기, Docker 실행, 기존 컨테이너 중지를 유발하지 않습니다. 실제 호스트 변경에는 별도의 `dure admin apply`가 필요합니다. 다중 노드 후보는 네트워크·NCCL 증적 기능이 추가되기 전까지 실패 안전 방식으로 거부됩니다.
